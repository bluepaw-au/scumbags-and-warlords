<script lang="ts">
    ...
    let { player, playTrick }: Props = $props();
    ...
    let cardStyle = $state((i:number) => {
        ...
        let diff = totalCardWidth - document.body.scrollWidth - 64;
...
<div class="hand-container fixed bottom-0 left-0 right-0 h-auto flex flex-col justify-end flex-1">
    ...
    <div class={"action-bar p-4 flex justify-center gap-2" + (player.hand.length == 0 ? " hidden" : "")}>
        <Button onAction={() => { tryTrick(player, selectedCards) } } disabled={selectedCards.cards.length < 1}> Play Trick </Button>
    </div>
